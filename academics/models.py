...
# Legacy Note model - keeping for migration compatibility
class Note(models.Model):
    """Legacy Note model - will be migrated to AcademicResource"""
    ...
    file = models.FileField(upload_to='legacy_notes/')
    
    note_type = models.CharField(max_length=20, choices=NOTE_TYPE_CHOICES, default='module')
    module_number = models.PositiveIntegerField(choices=MODULE_CHOICES, default=1)
...
def note_upload_path(instance, filename):
    """Legacy upload path function for compatibility"""
    return f"legacy_notes/{filename}"
