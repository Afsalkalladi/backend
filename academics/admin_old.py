...
class AcademicCategoryAdmin(admin.ModelAdmin):
    list_display = ['name', 'category_type', 'slug', 'is_active', 'display_order', 'resource_count']
...
class AcademicResourceAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'file_size_display', 'download_count', 'created_at'
    ]
    list_filter = [
    ...
    fieldsets = (
        ...
        ('File Information', {
            'fields': ('file', 'file_info', 'file_size')
        }),
        ('Resource Details', {
            'fields': ('module_number', 'exam_type', 'exam_year')
        }),
        ('Textbook Details', {
            'fields': ('author', 'publisher', 'edition', 'isbn'),
            'classes': ('collapse',)
        }),
        ('Approval Status', {
        ...
        ('Display & Statistics', {
            'fields': ('is_featured', 'is_active', 'download_count', 'view_count')
        }),
        ('Metadata', {
    ...
    actions = ['approve_resources', 'make_featured', 'remove_featured']
    
    def approve_resources(self, request, queryset):
        updated = 0
        for resource in queryset.filter(is_approved=False):
        ...
        self.message_user(request, f'Successfully approved {updated} resources.')
    approve_resources.short_description = "Approve selected resources"
    
    def make_featured(self, request, queryset):
        updated = queryset.update(is_featured=True)
        self.message_user(request, f'Successfully made {updated} resources featured.')
    make_featured.short_description = "Mark as featured"
    
    def remove_featured(self, request, queryset):
        updated = queryset.update(is_featured=False)
        self.message_user(request, f'Successfully removed featured status from {updated} resources.')
...
# Legacy Note admin - for migration support
@admin.register(Note)
class LegacyNoteAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Migration Notice', {
            'description': 'This is a legacy model. New resources should use AcademicResource model.',
            'fields': ()
        }),
        ('Basic Information', {
            'fields': ('title', 'description', 'subject', 'note_type', 'module_number')
        }),
        ('File', {
            'fields': ('file',)
        }),
